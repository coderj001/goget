...
[![Build Status](https://img.shields.io/github/checks-status/coderj001/goget/master?color=black&style=for-the-badge&logo=github)][github-actions]
...
[![License](https://img.shields.io/github/license/coderj001/goget?color=red&style=for-the-badge)][project-license]
[![Go v1.18](https://img.shields.io/badge/Go-%20v1.18-black?style=for-the-badge&logo=go)][gomod-file]

goget non-interactive file downloader command similar to wget
...
```sh
git clone https://github.com/coderj001/goget
cd goget
```
</details>
...
This section will guide you to setup a fully-functional local copy of `goget`
...
from [`goget's` Makefile][makefile-file] (most of which are
OS-independent)!

### Installing dependencies

To install all dependencies associated with `goget`, run the
command
...
Tests in `goget` are classified as *fast* and *slow* - depending
...
|      `run`     	| Run *goget*                           	|       NA      	|
|  `docker-gen`  	| Create production docker image for *goget* 	|     docker    	|
|  `docker-debug`  	| Create debug-friendly docker image for *goget* 	|     docker    	|
...
```text
goget_<version>_<target-os>_<architecture>.<extension>
```
...
```sh
goget_v1.2.1_windows_x86_64.exe
```
...
To run `goget` in a docker container, read the instructions in
[docker section](./docker).

### Running `goget`

To run goget, use the command
...
`goget` is powered by a template generated using [`go-template`][go-template-link]
