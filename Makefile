...
.PHONY: install
## `install`: Fetch dependencies needed to run `goget`
install:
...
.PHONY: run
## :
## `run`: Run `goget` in production mode
...
.PHONY: run-debug
## `run-debug`: Run `goget` in debug mode
...
.PHONY: docker-gen
## :
## `docker-gen`: Create a production docker image for `goget`
docker-gen:
...
.PHONY: docker-debug
## `docker-debug`: Create debug-friendly docker images for `goget`
docker-debug:
	echo "Building docker image \`$(IMAGE):$(VERSION)\`..."
