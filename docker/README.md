...
```bash
make docker-gen IMAGE=goget VERSION=latest
```
...
```bash
make clean-docker IMAGE=goget VERSION=latest
```


## Debug Images

The [dockerfile](./Dockerfile) for *goget* uses
...
```bash
make docker-debug IMAGE=goget-debug VERSION=latest
```
...
```bash
make clean-docker IMAGE=goget-debug VERSION=latest
```
